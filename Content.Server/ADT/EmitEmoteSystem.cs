...
public sealed class EmitEmoteSystem : SharedEmitEmoteSystem {
    ...
    private void TryEmitEmote(EntityUid uid, BaseEmitEmoteComponent component, EntityUid? user = null, bool predict = true)
    {
        ...
        if (prototype != null)
            if (prototype.ID.ToLower() == "adtdumbbell")
            {
                ...
                if (chatSystem != null && user != null)
                {
                    ...
                    //chatSystem.TryEmoteWithChat((EntityUid) user, "Scream"); Resources\Prototypes\Voice\speech_emotes.yml
                    chatSystem.TrySendInGameICMessage((EntityUid) user, "качается", InGameICChatType.Emote, false);
                }
            }
    }
}
